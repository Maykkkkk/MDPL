...
class Fusion(nn.Module):
    def __init__(self, channel, reduction=8):
        ...
        self.relational = nn.Sequential(
            nn.Conv2d(channel * 2, channel // reduction, kernel_size=1, groups=reduction, bias=True),
    ...
    def forward(self, x, inter):
        ...
        concat = torch.cat([x, inter], dim=1)
        rel_weight = self.relational(concat)
        rel_out = x * rel_weight


        blur_strength = self.blur_estimator(inter)
        blur_out = x * blur_strength


        out = rel_out + blur_out
        out = x + out
        return out
class Embeddings(nn.Module):
    def __init__(self):
        ...
        self.en_prompt3 = Fusion(320)

    def forward(self, x , inter, inter_down1, inter_down2):
        hx = self.en_layer1_1(x)
        hx = self.en_prompt1(hx, inter)
        ...
        hx = self.en_prompt2(hx, inter_down1)
        ...
        hx = self.en_prompt3(hx, inter_down2)
...
class Embeddings_output(nn.Module):
    ...
    def forward(self, x, residual_1, residual_2, inter, inter_down2):
        hx = self.de_prompt1(x, inter_down2)
        hx = self.de_layer3_1(hx)
...
class Stripformer(nn.Module):
    ...
    def forward(self, x, inter, inter_down1, inter_down2):
        hx, residual_1, residual_2 = self.encoder(x, inter, inter_down1, inter_down2)
        ...
        hx = self.decoder(hx, residual_1, residual_2, inter, inter_down2)
        return hx + x